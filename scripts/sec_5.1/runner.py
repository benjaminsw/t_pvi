...
def visualize_pdf(
    ...
    # Fix: Use the correct attribute to get edge colors
    # Option 1: Get color from the first line collection
    cs_color = []
    cs_color.append('black')  # c_true color
    for name, contour_set in cs.items():
        # Get the color from the first line collection
        if len(contour_set.allsegs) > 0:
            cs_color.append(colors[list(cs.keys()).index(name)])
        else:
            cs_color.append('blue')  # fallback color
    
    # Alternative Option 2: Directly use the colors you defined
    # cs_color = ['black'] + colors[:len(cs)]
